...
test.describe("Application Initialization and Navigation", () => {
	test("should load the application successfully", async ({ page }) => {
		...
		await expect(
			page.getByRole("link", { name: /Certificates/i }),
		).toBeVisible();

		// Verify Sites tab is active by default
		await expect(page.getByRole("link", { name: /Sites/i })).toHaveClass(
			/border-primary/,
		);
	...
	test("should navigate between main tabs", async ({ page }) => {
		...
		await expect(page.getByRole("link", { name: /Upstreams/i })).toHaveClass(
			/border-primary/,
		);
		...
		await expect(page.getByRole("link", { name: /Analytics/i })).toHaveClass(
			/border-primary/,
		);
		...
		await expect(page.getByRole("link", { name: /Certificates/i })).toHaveClass(
			/border-primary/,
		);
	...
	test.skip("should have no console errors on initial load", async ({
		page,
	}) => {
