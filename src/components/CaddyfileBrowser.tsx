  ...
  const handleFileClick = async (file: CaddyfileEntry) => {
    try {
      if (import.meta.env.DEV) {
        ...
        // In production, fetch from API
        const response = await fetch(`/api/caddyfiles/${encodeURIComponent(file.name)}`);
              ...
              <div className="flex-1 min-w-0">
                ...
                {file.stats && (
                  <div className="flex gap-3 mt-1 text-xs text-muted-foreground">
                    <span>{file.stats.siteBlocks} site block{file.stats.siteBlocks !== 1 ? 's' : ''}</span>
                    <span>•</span>
                    <span>{file.stats.directives} directive{file.stats.directives !== 1 ? 's' : ''}</span>
                  </div>
                )}
              </div>
            </button>
