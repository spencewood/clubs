...
export function validateCaddyfile(content: string): ValidationResult {
  ...
  const commonDirectives = [
    "root", "file_server", "reverse_proxy", "handle", "route",
    "encode", "log", "tls", "respond", "redir", "rewrite",
    "header", "basicauth", "request_header", "uri", "try_files",
    "php_fastcgi", "templates", "import", "vars", "bind", "acme_server"
  ];

  const directivePattern = new RegExp(`^\\s*(${commonDirectives.join("|")})\\b`, "i");
  ...
  for (const line of lines) {
    ...
    if (/^[a-z0-9.-]+\.[a-z]{2,}(\s|{|$)/i.test(trimmedLine) ||
    ...
    if (trimmedLine.includes("function(") || trimmedLine.includes("const ") ||
